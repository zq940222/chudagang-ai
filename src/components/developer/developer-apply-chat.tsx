...
export function DeveloperApplyChat() {
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6">
          <div className="mb-6 text-center">
            ...
            <h3 className="text-lg font-bold tracking-tight text-on-surface">
              {locale === "zh" ? "AI 辅助开发者认证" : "AI-Assisted Developer Certification"}
            </h3>
            <p className="mt-1 text-xs text-on-surface-variant leading-relaxed max-w-md">
              {locale === "zh"
                ? "告诉我你的专业背景和技能，我帮你快速完成认证"
                : "Tell me about your background and skills, I'll help you certify quickly"}
            </p>
          </div>
            ...
            <div className="relative group">
              ...
              <div className="relative flex items-center gap-3 glass rounded-2xl p-3 ghost-border shadow-2xl shadow-primary/5">
                ...
                <Input
                  ...
                  placeholder={locale === "zh" ? "介绍你的专业技能和经验..." : "Describe your skills and experience..."}
                  disabled={isBusy}
                  autoFocus
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2 max-w-2xl">
            {[
              locale === "zh" ? "我是全栈开发工程师，擅长 React 和 Node.js" : "I'm a full-stack developer skilled in React and Node.js",
              locale === "zh" ? "我是 AI/ML 工程师，专注深度学习" : "I'm an AI/ML engineer focused on deep learning",
              locale === "zh" ? "我是移动端开发，熟悉 Flutter 和 Swift" : "I'm a mobile developer, experienced with Flutter and Swift",
            ].map((prompt) => (
              <button
                key={prompt}
          ...
          <div key={message.id} className="space-y-2">
            {message.parts.map((part, i) => {
              switch (part.type) {
                ...
                case "reasoning":
                  return null;
                case "tool-extractDeveloperProfile":
                    ...
                    return (
                      ...
                      >
                        <p className="text-xs font-bold uppercase tracking-widest text-on-surface-variant/70">
                          {locale === "zh" ? "开发者资料摘要" : "Developer Profile Summary"}
                        </p>
                        <p className="text-base font-bold text-on-surface">{data.displayName}</p>
                        {data.title && <p className="text-sm text-on-surface-variant">{data.title}</p>}
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {data.skills.map((skill) => (
                            <span key={skill} className="px-2 py-0.5 rounded-full bg-secondary/10 text-xs font-medium text-secondary">
                              {skill}
                            </span>
                          ))}
                        </div>
                        {data.hourlyRate != null && (
                          <p className="text-sm text-on-surface-variant">
                            {data.currency ?? "CNY"} {data.hourlyRate}/hr
                          </p>
                        )}
                      </div>
                    );
                  ...
                  return null;
                case "tool-createDeveloperProfile":
                  if (part.state === "output-available") {
                    ...
                    if (result.error) {
                      return (
                        <div key={`${message.id}-tool-${i}`} className="rounded-lg bg-error/10 text-error px-3 py-2 text-xs">
                          {result.error}
                        </div>
                      );
                    }
                    return (
                      ...
                      >
                        ...
                        <p className="text-sm font-bold text-on-surface">
                          {locale === "zh" ? "资料已提交，AI 正在审核" : "Profile Submitted, AI Reviewing"}
                        </p>
                        <Button variant="secondary" size="sm" className="w-full" asChild>
                          <a href={`/${locale}/dashboard/developer`}>
                            {locale === "zh" ? "前往开发者面板" : "Go to Dashboard"}
                          </a>
                        </Button>
                      </div>
                    );
        ...
        <div className="relative group max-w-2xl mx-auto">
          ...
          <div className="relative flex items-center gap-2 glass rounded-xl p-2 ghost-border">
            <Input
              ...
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder={locale === "zh" ? "输入你的信息..." : "Type your info..."}
              disabled={isBusy}
              autoFocus
