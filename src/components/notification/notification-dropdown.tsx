...
export function NotificationDropdown({ onClose }: { onClose?: () => void }) {
  ...
  for (const n of notifications) {
    ...
    if (mins < 1) {
      timeLabels[n.id] = "just now";
    } else if (mins < 60) {
      timeLabels[n.id] = `${mins}m`;
    } else {
      const hours = Math.floor(mins / 60);
      if (hours < 24) {
        timeLabels[n.id] = `${hours}h`;
      } else {
        timeLabels[n.id] = `${Math.floor(hours / 24)}d`;
      }
    }
  }
          ...
          notifications.map((n) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <p
                  className={`text-sm ${!n.read ? "font-semibold" : "font-medium"} text-on-surface`}
                >
                  {n.title}
                </p>
